from datetime import datetime

from deepdiff import DeepDiff
import pandas as pd
from pandas import Timestamp

import mlops.best_practices_hw_6.homework.apps.batch as batch


def dt(hour, minute, second=0):
    return datetime(2023, 1, 1, hour, minute, second)


def test_prepare_data():

    expected_result = [
        {'PULocationID': '-1',
        'DOLocationID': '-1',
        'tpep_pickup_datetime': Timestamp('2023-01-01 01:01:00'),
        'tpep_dropoff_datetime': Timestamp('2023-01-01 01:10:00'),
        'duration': 9.0},
        {'PULocationID': '1',
        'DOLocationID': '1',
        'tpep_pickup_datetime': Timestamp('2023-01-01 01:02:00'),
        'tpep_dropoff_datetime': Timestamp('2023-01-01 01:10:00'),
        'duration': 8.0}
    ]

    # get actual result
    test_data = [
        (None, None, dt(1, 1), dt(1, 10)),
        (1, 1, dt(1, 2), dt(1, 10)),
        (1, None, dt(1, 2, 0), dt(1, 2, 59)),
        (3, 4, dt(1, 2, 0), dt(2, 2, 1)),      
    ]

    columns = ['PULocationID', 'DOLocationID', 'tpep_pickup_datetime', 'tpep_dropoff_datetime']
    df = pd.DataFrame(test_data, columns=columns)

    categorical = ['PULocationID', 'DOLocationID']
    actual_df = batch.prepare_data(df, categorical)

    actual_result = actual_df.to_dict(orient="records")

    diff = DeepDiff(expected_result, actual_result, significant_digits=1)
    assert len(diff) == 0

